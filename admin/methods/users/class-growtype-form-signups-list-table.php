 ...
 * @since 2.0.0
 */
class Growtype_Post_Signups_List_Table extends WP_Users_List_Table
{
    /**
     * Signup counts.
     *
    ...
    public function prepare_items()
    {
        ...
        $required_user_roles = !empty(get_option('Growtype_Post_default_user_role')) ? [get_option('Growtype_Post_default_user_role')] : ['subscriber'];
...
//            'meta_value' => '1',
        ...
        if (Growtype_Post_str_contains($search_value, '=')) {
...
//            'user_id' => __('User ID', 'growtype-post'),
            'username' => __('Username', 'growtype-post'),
            'name' => __('Name', 'growtype-post'),
            'email' => __('Email', 'growtype-post'),
            'registered' => __('Registration date', 'growtype-post'),
//            'count_sent' => __('Emails Sent', 'growtype-post')
        ));
    }
    ...
    {
        $actions = array (
            'activate' => _x('Evaluate', 'Registrations', 'growtype-post'),
            'resend' => _x('Email', 'Registrations', 'growtype-post'),
            'export_selected' => _x('Export selected', 'Registrations', 'growtype-post'),
            'export_all' => _x('Export all', 'Registrations', 'growtype-post'),
        );

        if (current_user_can('delete_users')) {
            $actions['delete'] = __('Delete', 'growtype-post');
        }
    ...
    {
        esc_html_e('No pending accounts found.', 'growtype-post');
    }
    ...
    public function column_cb($signup_object = null)
    {
        ...
        <label class="screen-reader-text" for="signup_<?php echo intval($signup_object->ID); ?>"><?php
            ...
            printf(esc_html__('Select user: %s', 'growtype-post'), $signup_object->user_login);
            ?></label>
    ...
    public function column_username($signup_object = null)
    {
        ...
        $email_link = add_query_arg(
            ...
            Growtype_Post_admin_url('users.php')
        );
        ...
        $actions['activate'] = sprintf('<a href="%1$s">%2$s</a>', esc_url($activate_link), __('Evaluate', 'growtype-post'));
//        $actions['resend'] = sprintf('<a href="%1$s">%2$s</a>', esc_url($email_link), __('Email', 'growtype-post'));

        if (current_user_can('delete_users')) {
            $actions['delete'] = sprintf('<a href="%1$s" class="delete">%2$s</a>', esc_url($delete_link), __('Delete', 'growtype-post'));
        }
    ...
    {
        $name = isset(Growtype_Post_Signup::get_signup_data($signup_object->ID)['first_and_last_name']) ?
            Growtype_Post_Signup::get_signup_data($signup_object->ID)['first_and_last_name']['value'] : $signup_object->user_name;
